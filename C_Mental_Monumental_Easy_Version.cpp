...
void solve(){
    ...
    for(int i=0;i<=n-1;i++){
        ...
        // check the element exists or not 
        auto it = find(all(a),i); if(it != a.end()){ int idx = it - a.begin(); used[idx] = true; continue; }


        // binary search for finding optimal solution of the current element
        int l=0,r=n-1,ans=-1;
        while(l <= r)
        {
            int mid=(l+r)/2;
        ...
        // find unused index starting from ans
        if(ans != -1){
            while(ans < n && used[ans]) ans++;
        }
